...
class Circuit():
    def __init__(self, povm='Tetra', Number_qubits=6 , latent_rep_size=100, gru_hidden=100,decoder='TimeDistributed_mol', Nsamples=0, init_state='0'):
        ...
        self.charset_length = self.povm.K  # K is the number of POVM basis
        self.K = self.povm.K
        ...
        self.sample_onehot, self.logP = self.generation(decoder, molecules=None,y0=self.y0) # generate sample and probability
        self.sample_RNN = tf.argmax(self.sample_onehot, axis=2) # get index


#        self.flipped_1qubit = tf.placeholder(tf.float32, [self.batchsize*self.K**1, self.max_length*self.charset_length])


        # TODO: why need update
        #  update
        self.gtype = tf.placeholder(tf.int64) # gate type, gtype=1 is 1-qubit, gtype=2 is 2-qubit
        f = tf.cond(tf.equal(self.gtype,1), lambda: self.K, lambda: self.K**2)
        self.flip_2 = tf.placeholder(tf.float32, [None, self.max_length*self.charset_length]) # flip configurationn
        self.f2 = tf.reshape(self.flip_2,[-1, self.max_length, self.charset_length]) # one-hot configuration
        self.co_2 = tf.placeholder(tf.float32, [None]) # operator coefficient
        self.probs,_ = self.generation(decoder, self.f2,y0=None, reu=True)
        self.V = tf.reduce_sum(self.f2 * tf.log(1e-10 + self.probs ), [1, 2]) ## self.f2 is one-hot encoding to get probability out
        self.cost = -tf.cast(f,tf.float32)*tf.reduce_mean(self.co_2 * self.V) ## compute sum O_{a,a'} logP(a) via sampling

        tf.summary.scalar('Cost_function', self.cost)

        # TODO: control, dependency, update and zip
    ...
    def generation(self,decoder, molecules=None,y0=None, reu=True):
        ...
        if decoder == 'TimeDistributed_mol':
            # molecules=None generate configuration and logP, otherwise it computes probablity of the given configuration
            # configuration is one-hot form, Nqubit * #POVM
            # output probability is one-hot shape, Nqubit * #POVM
            if molecules == None:
                b_size = tf.shape(y0)[0] # batch size
                with tf.variable_scope("generation", reuse=tf.AUTO_REUSE):
                    ...
                    z_matrix = tf.layers.dense(inputs=z, units=self.latent_rep_size, activation=tf.nn.relu,name="fc_GEN") ## this step may not be necessary or one can just parameterize directly
                # An atom of zeros. This is part of the (t=0) input of the RNN: input is concat([zmol, z_matrix]);
                y0 = tf.zeros([b_size, self.charset_length], dtype=tf.float32) # starting first zero for initializing P0 calculation

                logP = tf.zeros([b_size], dtype=tf.float32)

                # concatenates z_matrix to the atom of zeros as the first input of the RNN
                y_z = tf.concat([y0, z_matrix], axis=1) ## starting zero + latent_representation

                # initial state of the RNN during the unrolling

                s0 = tf.zeros([self.Ngru,b_size,2*self.gru_hidden], dtype=tf.float32) # LSTM

                #output tensor unrolling
                h2 = tf.zeros([b_size, 1, self.charset_length], dtype=tf.float32) ## output configuration
...
#
                ...
                def b(i, s0, h2, y_z,logP):
                    with tf.variable_scope("generation", reuse=True):
                        ...
                        dlayer = tf.layers.dense(outputs, kernel_initializer=self.initW, bias_initializer=self.initBias,units=self.charset_length, activation=tf.nn.softmax, name="output") ## output 4 probablity corresponding to y0


                        ## samples dlayer probabilities
                        logits  = tf.log(dlayer) ## take log probability of the output
                    ...
                    return [i+1,s0,tf.concat([h2,tf.reshape(dlayer,[b_size,1, self.charset_length])],axis=1),tf.concat([dlayer, z_matrix],axis=1),logP] ## concatenate output configuration dlayer to h2, and also put dlayer to z_matrix as the next input
                ...
                h2=tf.slice(h2, [0, 1, 0], [-1, -1, -1])  ## cuts the initial zeros that inserted to start the while_loop, which gets away the first initizalizing input y0 and z_matrix
                ...
                with tf.variable_scope("generation", reuse=reu):
                    ...
                    ## molecules is of shape: (batch, Nqubit, 4)
...
#
        return h2,logP
        ## for molecule == None, h2 is configuration and logP is corresponding probability; for molecule != None, h2 is the conditional probability given the molecule configuration
#
#
