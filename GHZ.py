...
# S: (batchsize, Nqubit), O: gate, K: POVM basis
# goal: given a', find connnect a
...
j_init = int(sys.argv[1])  # j_init=0 from scratch, others read model
...
a = (np.array(list(it.product(range(circ.K), repeat = 2)),dtype=np.uint8))  # Cartesian product from (0,0) to (3,3)
...
with tf.Session() as sess:
    ...
    if Ndataset != 0:
        ...
        samples,lP = sess.run([circ.sample_RNN,circ.logP],feed_dict={circ.y0: y0}) # circ.y0 is placeholder
        lP =np.reshape(lP,[-1,1]) # reshape to be array

        # get samples through for loop and store in cpu
    ...
    ## this part is to attain samples from the trained ansatz and prepare for fidelity calculation
    if Ndataset != 0:
        Ncalls = Ndataset /Nsamples
        y0 =  np.zeros((Nsamples,circ.K))
        samples,lP = sess.run([circ.sample_RNN,circ.logP],feed_dict={circ.y0: y0})
